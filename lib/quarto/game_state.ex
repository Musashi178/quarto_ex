defmodule Quarto.GameState do
  ...
  @doc """
    is_valid_move? for an initial game state will only return true if row and column are nil and next_stone is a valid game stone
  """
  def is_valid_move?(%GameState{stone_to_set: nil}, %Move{row: nil, column: nil, stone: next_stone}) do
    Enum.member?(Stone.get_all_stones, next_stone)
  end

  @doc """
    is_valid_move? for a non initial game state will only return true if row and column are set and the stones are valid
  """
  def is_valid_move?(%GameState{stone_to_set: %Stone{} = stone_to_set, board: board}, %Move{row: row, column: column, stone: %Stone{} = next_stone})
    when not is_nil(stone_to_set) and row in 1..4 and column in 1..4 do
      Board.is_field_empty?(board, row, column)
      and not Board.is_stone_set?(board, next_stone)
      and stone_to_set != next_stone
      and Enum.member?(Stone.get_all_stones, next_stone)
  end

  def is_valid_move?(_state, _move) do
    false
  end

  def apply_move(state, move) do
    if is_valid_move?(state, move) do
      new_state = state
